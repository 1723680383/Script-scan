    ...
    try:
        ...
        if response.status_code == 200:
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                if "没有指定抓取源" in response.text:
                                    res += (f"URL: {target_url}, 状态码: 200, 响应: 1.4.3\n")
                                ...
                                if "state" in response.text and "未知错误" in response.text:
                                    res += (f"URL: {target_url}, 状态码: 200, 响应: 1.3.6\n")
                                ...
                                if '"message"' in response.text and "UEditor" not in response.text :
                                    if "documentElement" not in response.text:                                 
                                        res += (f"URL: {target_url}, 状态码: 200, 响应: true\n")
                                ...
                                if "UEditor" in response.text:
                                    res += (f"URL: {target_url}, 状态码: 200, 响应: Ueditor.all.min.js文件存在\n")
                                    print(f"URL: {target_url}, 状态码: 200, 响应: Ueditor.all.min.js文件存在\n")
                                    print(f"{Fore.GREEN}Ueditor配置文件存在，请手动确认是否为变种！！！{Style.RESET_ALL}")
                                    global_main_paths_not_found  = False    

                                #查找CKfinder
                                if "CKFinder.version" in response.text:
                                    res += (f"URL: {target_url}, 状态码: 200, 响应: CKFinder.js文件存在\n")
                                    print(f"URL: {target_url}, 状态码: 200, 响应: CKFinder.js文件存在\n")
                                    print(f"{Fore.GREEN}CKFinder配置文件存在，请手动寻找上传位置！！！{Style.RESET_ALL}")

                                #查找CuteEditor
                                if "<configuration>" in response.text:
                                    res += (f"URL: {target_url}, 状态码: 200, 响应: CuteEdito本地包含漏洞存在\n")
                                    print(f"URL: {target_url}, 状态码: 200, 响应: CuteEdito本地包含漏洞存在\n")
                                    print(f"{Fore.GREEN}CuteEdito本地包含漏洞存在，成功读取到web.config的内容{Style.RESET_ALL}")
                                    global_main_paths_not_found  = False 

                                #查找Ckeditor
                                if "ckeditor.com" in response.text:
                                    res += (f"URL: {target_url}, 状态码: 200, 响应: ckeditor.js文件存在\n")
                                    print(f"URL: {target_url}, 状态码: 200, 响应: ckeditor.js文件存在\n")
                                    print(f"{Fore.GREEN}Ckeditor配置文件存在，请手动寻找上传位置！！！{Style.RESET_ALL}")       
            ...
            if global_main_paths_not_found:
                ...
                res += js_finder.get_js_paths(url, proxy=proxy)  # 传递代理参数
                # 继续检测
                    ...
                    for payload in payloads:
                        ...
                        try:
                            if response.status_code == 200:
                                #查找Ueditor
                                if "没有指定抓取源" in response.text:
                                    res += (f"URL: {complete_url}, 状态码: 200, 响应: 1.4.3\n")
                                    print(f"URL: {complete_url}, 状态码: 200, 响应: 1.4.3\n")
                                    print(f"{Fore.GREEN}Ueditor 漏洞存在！！版本为 - 1.4.3{Style.RESET_ALL}")

                                #查找Ueditor
                                if "state" in response.text and "未知错误" in response.text:
                                    res += (f"URL: {complete_url}, 状态码: 200, 响应: 1.3.6\n")
                                ...
                                if '"message"' in response.text and "UEditor" not in response.text:
                                    if "documentElement" not in response.text:    
                                        res += (f"URL: {complete_url}, 状态码: 200, 响应: true\n")
                                        print(f"URL: {complete_url}, 状态码: 200, 响应: 爆破成功！！！\n")
                                        print(f"{Fore.GREEN}Kindeditor 漏洞存在！！{Style.RESET_ALL}")

                                #查找Ueditor
                                if "UEditor" in response.text:
                                    res += (f"URL: {complete_url}, 状态码: 200, 响应: Ueditor.all.min.js文件存在\n")
                                    print(f"URL: {complete_url}, 状态码: 200, 响应: Ueditor.all.min.js文件存在\n")
                                    print(f"{Fore.GREEN}Ueditor配置文件存在，请手动确认是否为变种！！！{Style.RESET_ALL}")
                                ...
                                #查找Ckeditor
                                if "ckeditor.com" in response.text:
                                    res += (f"URL: {complete_url}, 状态码: 200, 响应: ckeditor.js文件存在\n")
                                    print(f"URL: {complete_url}, 状态码: 200, 响应: ckeditor.js文件存在\n")
                                    print(f"{Fore.GREEN}Ckeditor配置文件存在，请手动寻找上传位置！！！{Style.RESET_ALL}")
                                    global_main_paths_not_found  = False      
