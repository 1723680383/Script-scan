    ...
    try:
        ...
        if response.status_code == 200:
                            ...
                            if response.status_code == 200:
                                ...
                                if '"message"' in response.text and "UEditor" not in response.text :
                                    if "documentElement" not in response.text:                                 
                                        ...
                                        print(f"\n[+] [{target_url}] 状态码: 200, 响应: 爆破成功！！！")
                                        print(f"{Fore.GREEN}Kindeditor 漏洞存在！！{Style.RESET_ALL}\n")

                                #查找Fckeditor
                                if 'File Uploader' in response.text:
                                    res.append(f"[+] [{target_url}] 状态码: 200, 响应: 版本未知  Fckeditor 编辑器存在！！\n")
                                    print(f"\n[+] [{target_url}] 状态码: 200, 响应: 版本未知  Fckeditor 编辑器存在！！\n")
                                    global_main_paths_not_found  = False
                                    #找到了编辑器就不用继续爬取
                                ...
                                if "UEditor" in response.text:
                                    ...
                                    print(f"\n[+] [{target_url}] 状态码: 200, 响应: Ueditor.all.min.js文件存在")
                                ...
                                if "CKFinder.version" in response.text:
                                    ...
                                    print(f"\n[+] [{target_url}] 状态码: 200, 响应: CKFinder.js文件存在\n")
                                ...
                                if "<configuration>" in response.text:
                                    ...
                                    print(f"\n[+] [{target_url}] 状态码: 200, 响应: CuteEdito本地包含漏洞存在")
                                ...
                                if "ckeditor.com" in response.text:
                                    ...
                                    print(f"\n[+] [{target_url}] 状态码: 200, 响应: ckeditor.js文件存在")
            ...
            if global_main_paths_not_found:
                ...
                res += js_finder.get_js_paths(url, proxy=proxy, find=find)  # 传递代理参数
                # 继续检测
